...
chkinvok(const char *s)
{
	...
	char invocation[PATH_MAX + 1];
	strncpy(invocation, basename, PATH_MAX);
	invocation[PATH_MAX] = '\0';
	if (invocation[0] == '-')
		memmove(invocation, invocation + 1, strlen(invocation));
#ifdef _WIN32
	char *extension = strrchr(invocation, '.');
